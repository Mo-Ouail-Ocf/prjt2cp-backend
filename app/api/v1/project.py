...
def read_owned_projects(db: Session = Depends(get_db), 
                        user_id: int = Depends(get_current_user)):
    return fetch_owned_projects(db, user_id)

@router.get("/user/participated", response_model=List[ProjectDisplay])
def read_participated_projects(db: Session = Depends(get_db), 
                               user_id: int = Depends(get_current_user)):
...
def update_project(project_id: int, project_update: ProjectUpdate, db: Session = Depends(get_db), 
    ...
    updated_project = update_existing_project(db, project_id, project_update.model_dump())
...
def read_pending_invitations(user_id: int= Depends(get_current_user), db: Session = Depends(get_db)):
