...
class Idea(Base):
    ...
    parent_idea_id = Column(Integer, ForeignKey(
        'ideas.idea_id'), nullable=True)  # For tracking idea expansions
    ...
    # For expansions
    expansions = relationship('Idea', backref=backref(
        'parent_idea', remote_side=[idea_id]))


class CombinedIdea(Base):
    __tablename__ = 'combined_ideas'
    combined_idea_id = Column(Integer, ForeignKey(
        'ideas.idea_id'), primary_key=True)
    source_idea_id = Column(Integer, ForeignKey(
        'ideas.idea_id'), primary_key=True)

    source_idea = relationship('Idea', foreign_keys=[
                               source_idea_id], backref='contributed_to_combinations')
